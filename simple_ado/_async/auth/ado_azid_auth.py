"""Azure Identity async authentication auth class."""
...
class ADOAsyncAzIDAuth(ADOAsyncAuth):
    """Azure Identity async auth."""
    ...
    async def get_authorization_header(self) -> str:
        ...
        if self.access_token is None or self.access_token.expires_on <= time.time() + 60:
            self.access_token = await self._credential.get_token(
