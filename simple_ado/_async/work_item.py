...
"""ADO work item wrapper with lazy loading and patching capabilities (async)."""
...
class ADOAsyncWorkItem:
    ...
    _data: dict[str, Any]
    _client: "ADOAsyncWorkItemsClient"
    _project_id: str
    _log: logging.Logger
    ...
    def __init__(
        self,
        data: dict[str, Any],
        client: "ADOAsyncWorkItemsClient",
        project_id: str,
        log: logging.Logger,
    ) -> None:
    ...
    async def __setitem_async__(self, key: str | ADOWorkItemBuiltInFields, value: Any) -> None:
        """Set a field value and patch it on the server.

        Equivalent to ``await work_item.patch(key, value)``.

        :param key: The field name or ADOWorkItemBuiltInFields enum value
        :param value: The new value for the field
        """
        await self.patch(key, value)

    async def set(self, key: str | ADOWorkItemBuiltInFields, value: Any) -> None:
        """Set a field value and patch it on the server.

        Convenience alias for ``await work_item.patch(key, value)``.
    ...
    async def refresh(self) -> None:
        ...
        self._data = await self._client.get(str(work_item_id), self._project_id)
    ...
    def __repr__(self) -> str:
        ...
        return f"<ADOAsyncWorkItem(id={work_item_id}, type='{work_item_type}')>"
