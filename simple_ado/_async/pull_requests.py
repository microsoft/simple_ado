...
"""ADO Pull Request API wrapper (async)."""
...
from simple_ado._async.git import ADOGitStatusState
from simple_ado._async.http_client import ADOAsyncHTTPClient, ADOResponse, ADOThread
...
class ADOAsyncPullRequestClient(ADOAsyncBaseClient):
    ...
    async def details(self) -> ADOResponse:
    ...
    async def workitems(self) -> ADOResponse:
    ...
    async def iterations(self) -> ADOResponse:
    ...
    async def get_threads(self, *, include_deleted: bool = False) -> list[ADOThread]:
    ...
    async def create_comment_with_text(
        self,
        comment_text: str,
        *,
    ...
    async def delete_thread(self, thread: ADOThread) -> None:
        ...
        for comment in thread["comments"]:
            ...
            await self.http_client.delete(request_url)

    async def create_thread_list(
        self,
        *,
        ...
        for thread in threads:
            ...
            await self.create_comment(thread, comment_identifier=comment_identifier)

    async def get_statuses(self) -> ADOResponse:
    ...
    async def threads_with_identifier(self, identifier: str) -> list[ADOThread]:
        ...
        for thread in await self.get_threads():
    ...
    async def delete_threads_with_identifier(self, identifier: str) -> None:
        ...
        for thread in await self.threads_with_identifier(identifier):
            self.log.debug(f"Deleting thread: {thread}")
            await self.delete_thread(thread=thread)

    async def get_properties(self) -> dict[str, PropertyValue]:
    ...
    async def patch_properties(self, operations: list[PatchOperation]) -> dict[str, PropertyValue]:
        ...
        response = await self.http_client.patch(request_url, operations=operations)
    ...
    async def add_property(self, name: str, value: str) -> dict[str, PropertyValue]:
        ...
        return await self.patch_properties([operation])

    async def delete_property(self, name: str) -> dict[str, PropertyValue]:
