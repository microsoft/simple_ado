...
"""ADO identities API wrapper (async)."""
...
class ADOAsyncIdentitiesClient(ADOAsyncBaseClient):
    ...
    async def search(self, identity: str) -> list[dict[str, Any]]:
    ...
    async def get_team_foundation_id(self, identity: str) -> TeamFoundationId:
        """Fetch the unique Team Foundation GUID for a given identity.
        ...
        results = await self.search(identity)
