...
from simple_ado._async.builds import BuildQueryOrder
...
async def test_get_builds(
    ...
    base_url = f"https://{mock_async_client.http_client.tenant}.visualstudio.com/DefaultCollection/{mock_project_id}/_apis/build/builds/"
    ...
    builds = []
    async for build in mock_async_client.builds.get_builds(project_id=mock_project_id):
        builds.append(build)
...
@pytest.mark.asyncio
@respx.mock
async def test_get_builds_with_definition_filter(
    mock_async_client: ADOAsyncClient, mock_project_id: str
) -> None:
    ...
    builds = []
    async for build in mock_async_client.builds.get_builds(
        project_id=mock_project_id, definitions=[100, 101]
    ):
        builds.append(build)
...
@pytest.mark.asyncio
@respx.mock
async def test_get_builds_with_order(
    mock_async_client: ADOAsyncClient, mock_project_id: str
) -> None:
...
@pytest.mark.asyncio
@respx.mock
async def test_build_info(mock_async_client: ADOAsyncClient, mock_project_id: str) -> None:
    """Test getting build info."""
    build_id = 12345
    base_url = f"https://{mock_async_client.http_client.tenant}.visualstudio.com/DefaultCollection/{mock_project_id}/_apis/build/builds/{build_id}"
    ...
    result = await mock_async_client.builds.build_info(
        project_id=mock_project_id, build_id=build_id
    )
...
@pytest.mark.asyncio
@respx.mock
async def test_queue_build(mock_async_client: ADOAsyncClient, mock_project_id: str) -> None:
    ...
    result = await mock_async_client.builds.queue_build(
...
@pytest.mark.asyncio
@respx.mock
async def test_list_artifacts(mock_async_client: ADOAsyncClient, mock_project_id: str) -> None:
    ...
    respx.get(
        url__startswith=f"https://{mock_async_client.http_client.tenant}.visualstudio.com/DefaultCollection/{mock_project_id}/_apis/build/builds/{build_id}/artifacts",
    ).mock(return_value=httpx.Response(200, json=artifacts_data))

    result = await mock_async_client.builds.list_artifacts(
        project_id=mock_project_id, build_id=build_id
    )
...
@pytest.mark.asyncio
@respx.mock
async def test_get_definitions(mock_async_client: ADOAsyncClient, mock_project_id: str) -> None:
    ...
    respx.get(
        url__startswith=f"https://{mock_async_client.http_client.tenant}.visualstudio.com/DefaultCollection/{mock_project_id}/_apis/build/definitions",
    ).mock(return_value=httpx.Response(200, json=definitions_data))

    result = await mock_async_client.builds.get_definitions(project_id=mock_project_id)
...
@pytest.mark.asyncio
@respx.mock
async def test_get_definition(mock_async_client: ADOAsyncClient, mock_project_id: str) -> None:
    ...
    respx.get(
        url__startswith=f"https://{mock_async_client.http_client.tenant}.visualstudio.com/DefaultCollection/{mock_project_id}/_apis/build/definitions/{definition_id}",
    ).mock(return_value=httpx.Response(200, json=definition_data))

    result = await mock_async_client.builds.get_definition(
...
@pytest.mark.asyncio
@respx.mock
async def test_delete_definition(mock_async_client: ADOAsyncClient, mock_project_id: str) -> None:
    ...
    await mock_async_client.builds.delete_definition(
        project_id=mock_project_id, definition_id=definition_id
    )
