...
def main():
    ...
    client = simple_ado.ADOClient(tenant="dalemyers", credentials=("", ""))
    ...
    output = (
        pathlib.Path(__file__).parent / pathlib.Path(outputDir) / pathlib.Path(repo_id + ".zip")
    )
    ...
    try:
        ...
        http = simple_ado.http_client.ADOHTTPClient(
            tenant=tenant, credentials=token, user_agent="test", log=logger
        )
        ...
        zip = git_client.download_zip(
            output_path=output, repository_id=repo["id"], branch=branch, project_id=project_id
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="python progress_callback_test.py", description="""Notes.""", epilog="""Notes."""
    )
    parser.add_argument(
        "-r", "--repo", type=str, dest="repoUrlStr", help="REQUIRED: Output directory."
    )
    parser.add_argument(
        "-o", "--output", type=str, dest="output", help="REQUIRED: Output directory."
    )
    parser.add_argument(
        "-p",
        "--progress",
        action="store_true",
        dest="progress",
        help="Turn on callbacks to show progress of long-running operations.",
    )
